...
- **No "Event" concept**: the original template's multi-event support (event picker, per-event data scoping, "Event" CSV column) was removed entirely. Hala isn't working conferences/booths, so every lead just lands in one shared list — there's no "which event are you at?" prompt and nothing to pick or add. Only the "Logging as [Name]" rep picker remains, in case an assistant ever helps capture leads.
- **My Card has separate Website and Instagram fields**: Hala's own outgoing digital card (My Card) has two dedicated, pre-filled fields instead of one combined/manual link — **Website** defaults to `www.loulouniche.com` and **Instagram** defaults to `@loulouniche` (both editable, both configurable via `APP_CONFIG.website`/`APP_CONFIG.instagram`). Both always appear as separate links on the vCard and the on-screen card output. (The unrelated "Instagram / Website" field on the *lead capture* form, for recording a lead's own info, is untouched.)
- **Firebase is live**: the real `loulouniche-lead-tracker` Firebase project is connected (Firestore created, security rules published as an open ruleset — `allow read, write: if true`, matching the app's no-login design). Verified end-to-end: leads, profiles, and My Card data all persist server-side and sync correctly across devices/browsers, not just local storage. Also fixed a startup race condition where the "your name" picker could open before Firestore's first sync arrived, showing an empty picker even with good data available.

**Still needed before this is fully live**:
1. Create/connect a Netlify site pointed at this repo and add the `ANTHROPIC_API_KEY` environment variable so business-card scanning works — see "Deploy to Netlify" below (requires a Netlify account, a step only Hala/Gabe can do).
2. Firestore rules are intentionally open right now (`allow read, write: if true`) because the app has no login — anyone with the deployed URL can read/write all lead data. Locking this down for real requires adding actual sign-in (e.g. Firebase Authentication) and changing rules to check `request.auth`; Firebase App Check alone would block bots/scripts but not a person who just opens the site. Revisit this on a more secure base later if needed.
